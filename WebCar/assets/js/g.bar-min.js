...
*/ (function() {
    ...
    function VBarchart(paper, x, y, width, height, values, opts) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (opts.stacked) {
                ...
                covers2.push(cvr = paper.rect(stack[0].x - stack[0].w / 2, y, barwidth, height)
                    .attr(chartinst.shim));
                cvr.bars = paper.set();
                ...
                for (var s = 0, ss = stack.length; s < ss; s++) {
                    ...
                    covers.push(cover = paper.rect(bar.x - bar.w / 2, bar.y, barwidth, bar.value * Y)
                        .attr(chartinst.shim));
                    cover.bar = bar;
        ...
        if (!opts.stacked) {
            for (var i = 0; i < len; i++) {
                for (var j = 0; j < (multi || 1); j++) {
                    ...
                    covers.push(cover = paper.rect(Math.round(X), y + barvgutter, barwidth, height - barvgutter)
                        .attr(chartinst.shim));
        ...
        chart.label = function(labels, isBottom) {
            ...
            if (opts.stacked) {
                for (var i = 0; i < len; i++) {
                    ...
                    for (var j = 0; j < (multi || 1); j++) {
                        ...
                        if (j == multi - 1) {
                            ...
                            L = paper.text(bars[j][i].x, y + height - barvgutter / 2, label)
            	...
            	console.log("label not stacked");
                for (var i = 0; i < len; i++) {
                    for (var j = 0; j < (multi || 1); j++) {
                        var label = chartinst.labelise(multi ? labels[j] && labels[j][i] : labels[i], multi ? values[j][i] : values[i], total);

                        L = paper.text(bars[j][i].x - barwidth / 2, isBottom ? y + height - barvgutter / 2 : bars[j][i].y - 10, label)
                            .attr(chartinst.txtattr)
                            .attr({
                            fill: opts.legendcolor || "#000",
                            "text-anchor": "start"
    ...
    function HBarchart(paper, x, y, width, height, values, opts) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (opts.stacked) {
                var cvr = paper.rect(x, stack[0].y - stack[0].h / 2, width, barheight)
                    .attr(chartinst.shim);
                ...
                for (var s = 0, ss = stack.length; s < ss; s++) {
                    ...
                    bar.w = val;
                    bar.x = x + val;
                    covers.push(cover = paper.rect(x + size * X, bar.y - bar.h / 2, bar.value * X, barheight)
                        .attr(chartinst.shim));
                    cover.bar = bar;
                    size += bar.value;
                }
        ...
        if (!opts.stacked) {
            for (var i = 0; i < len; i++) {
                for (var j = 0; j < (multi || 1); j++) {
                    var cover = paper.rect(x, Y, width, barheight)
                        .attr(chartinst.shim);
        ...
        chart.label = function(labels, isRight) {
            ...
            for (var i = 0; i < len; i++) {
                for (var j = 0; j < multi; j++) {
                    ...
                    this.labels.push(L = paper.text(X, bars[j][i].y, label)
                        .attr(chartinst.txtattr)
                        .attr({
                        fill: opts.legendcolor || "#000",
                        "text-anchor": "start"
